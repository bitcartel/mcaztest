...
su -l $AZUREUSER -c "sed -i \"s/^default-network-port =.*/default-network-port = 4333/\" $HOMEDIR/.multichain/${CHAINNAME}/params.dat"
su -l $AZUREUSER -c "sed -i \"s/^default-rpc-port =.*/default-rpc-port = 4332/\" $HOMEDIR/.multichain/${CHAINNAME}/params.dat"
